...
def player(positions, colors):
    p = [[ Int("p_%s_%s" % (i, j)) for i in range(colors)] for j in range(positions) ] # p[a][b] is true if color a is at position b
    beta = [[ Int("beta_%s_%s" % (i, j)) for i in range(positions + 1)] for j in range(colors) ] # beta[a][b] is min(sum_p[a], b)
    ...
    sum_p = [functools.reduce(lambda x, y: x+y, column_sum(i)) for i in range(colors)]  # alp[a] is the number of colors of a in RR
    cond3 = [beta[i][j] == min2(sum_p[i], j) for i in range(colors) for j in range(positions + 1) ]
    ...
    while True:
        ...
        if s.check() == unsat:
            ...
            if inp.upper() == "YES":
                ...
                cond3 = And(functools.reduce(lambda x, y: x+y, cond1) == correct_pos, functools.reduce(lambda x, y: x+y, cond2) == correct_num) 
                C.append(cond3)
